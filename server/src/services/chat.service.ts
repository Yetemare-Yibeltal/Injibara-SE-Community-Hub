...
function buildAccessFilter(ctx: AccessContext): FilterQuery<IChat> {
  const scopedFilter: FilterQuery<IChat> =
    ctx.allowedBatches === 'all'
      ...
      : { type: { $in: ['batch', 'course', 'department'] }, batchScope: { $in: ctx.allowedBatches } };
...
export async function listChatsForUser(ctx: AccessContext): Promise<IChat[]> {
  const filter = buildAccessFilter(ctx);
  return Chat.find({ ...filter, isArchived: false }).sort({ updatedAt: -1 });
}

export async function getChatById(chatId: string, ctx: AccessContext): Promise<IChat> {
...
async function userCanAccessChat(chat: IChat, ctx: AccessContext): Promise<boolean> {
...
export async function archiveChat(chatId: string, ctx: AccessContext): Promise<IChat> {
...
export async function toggleMuteChat(chatId: string, ctx: AccessContext): Promise<IChat> {
  const chat = await getChatById(chatId, ctx);
